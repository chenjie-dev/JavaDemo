...
//         future.complete("test");
     ...
     */
    @Test
    public void testCompletableFuture_Async_Combination3() throws ExecutionException, InterruptedException {
        // 创建异步执行任务:
        CompletableFuture<Double> job = CompletableFuture.supplyAsync(() -> {
            System.out.println(Thread.currentThread() + " start job1,time->" + System.currentTimeMillis());
            try {
        ...
        CompletableFuture<String> job2 = job.thenCompose((param) -> {
            System.out.println(Thread.currentThread() + " start job2,time->" + System.currentTimeMillis());
            try {
                Thread.sleep(2000);
            } catch (InterruptedException ignored) {
        ...
        System.out.println("job run result->" + job.get());
        System.out.println("main thread start job2.get(),time->" + System.currentTimeMillis());
        System.out.println("job2 run result->" + job2.get());
        System.out.println("main thread exit,time->" + System.currentTimeMillis());
    }
     ...
     */
    @Test
    public void testCompletableFuture_Async_Combination4() throws ExecutionException, InterruptedException {
        // 创建异步执行任务:
        CompletableFuture<Double> job = CompletableFuture.supplyAsync(() -> {
            System.out.println(Thread.currentThread() + " start job1,time->" + System.currentTimeMillis());
            try {
        ...
        CompletableFuture<Double> job3 = CompletableFuture.supplyAsync(() -> {
            System.out.println(Thread.currentThread() + " start job3,time->" + System.currentTimeMillis());
            try {
                Thread.sleep(1300);
            } catch (InterruptedException ignored) {
        ...
        System.out.println("main thread start job4.get(),time->" + System.currentTimeMillis());
        //等待子任务执行完成
        System.out.println("job4 run result->" + job4.get());
        System.out.println("main thread exit,time->" + System.currentTimeMillis());
    }
